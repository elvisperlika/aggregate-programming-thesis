...
In figura \cref{fig:movement-simulations} sono state simulate le funzioni \verb|goTo|, \verb|explore| \\ e \verb|obstacleAvoidance| su Alchemist, un simulatore di sistemi aggregati.
...
Tutte e tre le formazioni \cref{fig:formation-simulations} si possono muovere nello spazio e necessitano di un leader che può anche variare velocità, in quel caso i nodi vicini si adatteranno alla velocità del leader. Questo tipo di formazioni sono capaci di ricostruirsi anche nel caso cambi il numero di nodi oppure un fenomeno esterno vada a disturbarne la struttura.
...
Nel nostro contesto si è sviluppato un server che comunica con due client\cref{fig:server-arc}:
\begin{itemize}
