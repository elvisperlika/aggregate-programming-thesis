...
L'obiettivo di questa tesi è dimostrare come la macroprogrammazione, e quindi le tecnologie che ne derivano, come il framework ScaFi e la sua estensione MacroSwarm, risultino essere un paradigma di programmazione particolarmente efficace nella la progettazione di sistemi aggregati eterogenei. 
L'esigenza di framework di questo tipo è dovuta alla sempre più frequente necessità di creare comportamenti collettivi in modo intuitivo, astraendo dalla logica dei singoli componenti del insieme da controllare.
Nel contesto della progettazione di sistemi aggregati, l'Edge Computing è l'approccio più utilizzato, soprattutto nel mondo IoT per via del buon compromesso tra capacità computazionale e velocità di comunicazione tra i nodi della rete. Risulta però non in grado di identificare una qualche astrazione di ``Intelligenza Collettiva Emergente'' per via della sua natura fortemente concentra sull'aspetto funzionale dei singoli dispositivi. 
La volontà di colmare questa lacuna e quindi di poter creare un intelligenza collettiva in modo efficace, intuitivo e che permetta di astrarre dalla tecnologia dei dispositivi dell'insieme su cui si vuole definire la strategia operativa ha dato origine al mondo dell'Aggregate Programming. I contesti di applicazione di queste tecnologie sono molteplici, ad esempio il controllo di sensori IoT per smart city oppure il controllo di droni in formazione per missioni di salvataggio.
...
L' \ac{AP} ha permesso la creazione di framework come ScaFi, Protelis e MacroSwarm. Alla base di questo approccio c'é concetto dei sopracitati \textit{campi di calcolo}. Per \textit{campo di calcolo} o \textit{computationals field} si intende la mappatura degli elementi del dominio in cui si opera in un insieme di valori.
